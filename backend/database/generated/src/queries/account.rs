...
pub fn next_donatable_date() -> NextDonatableDateStmt {
    NextDonatableDateStmt(crate::client::async_::Stmt::new(
        "SELECT COALESCE(( SELECT CASE WHEN ( donations.created_at + CASE WHEN donations.type = 'whole_blood' THEN INTERVAL '56 days' WHEN donations.type = 'power_red' THEN INTERVAL '112 days' WHEN donations.type = 'platelet' THEN INTERVAL '7 days' WHEN donations.type = 'plasma' THEN INTERVAL '28 days' ELSE INTERVAL '0 days' END ) <= now() THEN now() ELSE ( donations.created_at + CASE WHEN donations.type = 'whole_blood' THEN INTERVAL '56 days' WHEN donations.type = 'power_red' THEN INTERVAL '112 days' WHEN donations.type = 'platelet' THEN INTERVAL '7 days' WHEN donations.type = 'plasma' THEN INTERVAL '28 days' ELSE INTERVAL '0 days' END ) END FROM donations WHERE ( SELECT member_id FROM appointments WHERE id = donations.appointment_id ) = $1 ORDER BY donations.created_at DESC LIMIT 1 ), now()) AS next_donatable_date",
    ))
}
