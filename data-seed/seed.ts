...
const START_DATE = dayjs().subtract(3, 'year').toDate();
...
const BLOOD_COMPATIBILITY: Record<string, string[]> = {
  o_minus: ['o_minus', 'o_plus', 'a_plus', 'a_minus', 'b_plus', 'b_minus', 'ab_plus', 'ab_minus'],
...
const APPT_STATUSES = ['on_process', 'approved', 'checked_in', 'donated', 'done', 'rejected'] as const;
const DONATION_TYPES = ['whole_blood', 'power_red', 'platelet', 'plasma'] as const;
...
async function seed() {
  ...
  const tables = [
    'blood_bags', 'donations', 'healths', 'answers', 'appointments', 'request_blood_groups', 'blood_requests',
    'blog_tags', 'comments', 'blogs', 'tags', 'questions', 'accounts'
  ];
  ...
  for (const q of questions) {
    const res = await client.query(`INSERT INTO questions (content, is_active) VALUES ($1,true) RETURNING id`, [q]);
  ...
  for (let i = 0; i < 2000; i++) {
    ...
    await client.query(
      ...
      [id, role, email, password, phone, name, gender, address, birthday, blood_group, created_at]
    );
  ...
  for (const staffId of staffIds) {
    for (let i = 0; i < 3; i++) {
      ...
      const end = dayjs(start).add(faker.number.int({ min: 1, max: 10 }), 'day').toDate();
      const created_at = randomDate(START_DATE, END_DATE);
      ...
      await client.query(
        ...
        [id, staffId, priority, title, max_people, start, end, isActive, created_at]
      );
      ...
      const bloodGroups = faker.helpers.uniqueArray(BLOOD_GROUPS, faker.number.int({ min: 1, max: 4 }));
      requestBloodMap[id] = new Set(bloodGroups);
      for (const group of bloodGroups) {
        await client.query(`INSERT INTO request_blood_groups (request_id, blood_group) VALUES ($1,$2)`, [id, group]);
      }
    }
  }
  ...
  for (const requestId of requestIds) {
    ...
    for (const donorId of donors) {
      ...
      const compatible = BLOOD_COMPATIBILITY[donorBlood].some(bg => allowedBloods.has(bg));
      if (!compatible) continue;

      donorActiveApptMap[donorId] = true;
      const id = uuidv4();

      const temp = faker.number.float({ min: 36.0, max: 37.5, fractionDigits: 1 });
      ...
      let status = good ? pick(APPT_STATUSES.filter(s => s !== 'rejected')) : 'rejected';
      if (!isActive && ['on_process', 'approved', 'checked_in'].includes(status)) status = 'done';
      if (status === 'donated' && !currentActiveRequests.includes(requestId)) continue;

      const reason = status === 'rejected' ? pick([
      ...
      requestPeopleMap.set(requestId, requestPeopleMap.get(requestId)! + 1);
      ...
      await client.query(
        ...
        [uuidv4(), id, temp, weight, sys, dia, hr, good, good ? 'All vitals normal' : 'Vitals slightly off', randomDate(START_DATE, END_DATE)]
      );
      ...
      if (status !== 'rejected') {
        ...
        let tries = 0;
        while (dayjs(donationDate).diff(last, 'day') < DONATION_INTERVALS[donationType] && tries < 10) {
        ...
        await client.query(`INSERT INTO donations (id, appointment_id, type, amount, created_at)
          VALUES ($1, $2, $3, $4, $5)`,
          [donationId, id, donationType, amount, donationDate]);

        for (const comp of COMPONENTS) {
          let expired;
          if (comp === 'plasma') expired = dayjs(donationDate).add(365, 'day').toDate();
          else if (comp === 'platelet') expired = dayjs(donationDate).add(5, 'day').toDate();
          else expired = dayjs(donationDate).add(42, 'day').toDate();

          const isUsed = expired < new Date();
          await client.query(`INSERT INTO blood_bags (id, donation_id, component, is_used, amount, expired_time)
            VALUES ($1, $2, $3, $4, $5, $6)`,
            [uuidv4(), donationId, comp, isUsed, comp === 'red_cell' ? 300 : 100, expired]);
        }
      }
    }
  }
  ...
  for (const name of tagNames) {
    const id = uuidv4();
    await client.query(`INSERT INTO tags (id, name) VALUES ($1, $2)`, [id, name]);
    tagIds.push(id);
  }
  ...
  for (let i = 0; i < 100; i++) {
    ...
    const usedTags = faker.helpers.uniqueArray(tagIds, faker.number.int({ min: 1, max: 3 }));
    for (const tagId of usedTags) {
      await client.query(`INSERT INTO blog_tags (blog_id, tag_id) VALUES ($1, $2)`, [blogId, tagId]);
    }
